...
/*!
  ...
  It also runs the model and returns the selected results.
*/
struct ABM{
    explicit ABM(py::dict agent_modelObjs, py::object patch_model, settings_t settings) {
        /**
         * Receives user defined settings and creates Frame object
         */
    ...
    py::dict run() {
        /**
        * Calls Frame::setup to setup ABM and then Frame::run to run the model. 
        * It receives the results as a py::dict and returns.
        **/
        model->setup();
